    ...
    const setContent = function({root, menu}) {
        root.innerHTML = `
            <div style="overflow-x: auto; margin: 10px 0px; display:flex; justify-content: center;">
                <canvas id="task8-canvas" width="auto" height="200" style="border:1px solid #000;">
                </canvas>
            <div>
        `;
        ...
        setInterval(() => {
            context.beginPath();
            context.moveTo(posX, posX / 2);
            context.lineTo(
                (posX + Math.cos(angle * Math.PI / 180) * 60), 
                ((posX / 2) + Math.sin(angle * Math.PI / 180) * 60)
            )
            ...
            context.strokeStyle = posX % 8 === 0 ? '#5290e2' : '#3d5a80';
            context.stroke();
