...
document.getElementById('task2_4-button').onclick = function() {
    ...
    */
    ...
    setContent({
        element: document.getElementById('task2_4-content'),
        text: sliceText(window.prompt('Введите текст:'),
            parseInt(window.prompt('Кол-во символов: ', 10)))
    });
};
document.getElementById('task2_5-button').onclick = function() {
    ...
    */
    const setContent = ({ element, text }) => element.innerHTML = text;
    setContent({
        element: document.getElementById('task2_5-content'),
        text: changeSymbols(window.prompt('Введите текст:'), { from: '-', to: '!' })
    });
};
document.getElementById('task3_1-button').addEventListener('click', function(event) {
    window.alert(`Кнопка: ${this.innerText}`);
});
document.getElementById('task3_2-button').onclick = function() {
    ...
    createElement({
        root: document.getElementById('task3_2-content'),
        callback: (value) => window.alert(`Площадь: ${calculateCircleArea(value)}`) 
    })
}
document.getElementById('task3_3-button').onclick = function() {
    /**
    ...
    createElement({
        root: document.getElementById('task3_3-content'),
