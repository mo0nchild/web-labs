...
document.getElementById('task5-button').onclick = function() {
    ...
    const setContent = function({root}) {
        ...
        <form style="margin: 10px 0px; padding: 10px;" >
            ...
            <input style="margin: 5px 0px" type="submit" value="Изменить"/>
        </form>
        `;
        root.querySelector('form').addEventListener('submit', function(event) {
            event.preventDefault();
    ...
    */
    const setContent = function({root, images}) {
        ...
        <form style="margin: 10px 0px; padding: 10px;" >
            <label>Выберите цвет:</label>
            <select name="image">
                <option value="0" selected>Странный тюлень</option>
                <option value="1">Красивый тюлень</option>
                <option value="2">Гордый тюлень</option>
            </select>
            <input style="margin: 5px 0px" type="submit" value="Изменить"/>
        </form>
...
document.getElementById('task7-button').onclick = function() {
    ...
    */
    const setContent = function({root, images}) {
        root.innerHTML = `
        <div style="margin: 10px 0px;">
            <button data-back style="display: inline-block;">Назад</button>
            <button data-next style="display: inline-block;">Вперед</button>
        </div>
        <img style="width: 200px; height: 200px; object-fit: cover;" alt="..." />
        `;
